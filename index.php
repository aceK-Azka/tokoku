...
if($websitetitle == ""){
	?>
	Selamat datang! Buka halaman ini sekali lagi. Jika Anda masih melihat pesan ini, itu berarti Anda tidak mengatur konfigurasi dengan benar di file config.php
	<?php
}else{
		...
		<body>
			...
			//Home
				...
				<div class="section gridcontainerunscrollable">
					<?php
					$sql = "SELECT * FROM $tableposts JOIN $tableshop ON $tableposts.tokoid = $tableshop.id ORDER BY $tableposts.id DESC";
					$result = mysqli_query($connection, $sql);
					if($result){
						if(mysqli_num_rows($result) > 0){
							$productindex = 0;
			...
			?>
			<!-- TEMPORARILY DISABLED for future update -->
			...
			<script>
				...
				function addtocart(n, num){

					var prod = $(".filmblock").eq(n) // Take HTML Elements based on .eq(n)
					...
					// Add product infos to cartobject array
					function pushit(){
						cartobject.push({
							id : n,
							title : prodtitle,
					...
					if(cartobject.length == 0){
						pushit()
						// Added one TEMPORARY function : showcartui() for early testing
						showcartui()
					}else{
						for(var i = 0; i < cartobject.length; i++){
				...
				function chatnow(){
					...
					if(cdname != "" && cdmobile != "" && cdaddress != "" && cdmethod != ""){
						...
						$.post("<?php echo $baseurl ?>ordernotes.php", {
							...
							window.open("https://wa.me/" + cartobject[0].shop_number + "?text=" + omuri, '_blank');
						})

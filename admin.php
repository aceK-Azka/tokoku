<?php
/*
Developed by Twentytwo-aio
Email: twentytwoaio@gmail.com 
WhatsApp: 628889990010
WebSite: https://admin.aiohost.xyz
*/
					...
					<div style="display: table-row; height: 100%;">
						<div class="adminmenubar">
							<div class="stickythingy">
								...
								<a href="<?php echo $baseurl ?>admin.php?shop"><div class="adminleftbaritem"><i class="fa fa-tag" style="width: 30px;"></i> <?php echo uilang("Shop") ?></div></a>
								...
								<!-- <div style="text-align: center; padding: 30px; font-size: 10px;"><?php echo uilang("Developed by") ?><br><a target="_blank" class="textlink" style="color: lime;" href="https://twentytwo-aio.xyz/">Twentytwo-aio</a><br><br>Donate to the author:<br><a href="https://admin.aiohost.xyz" class="textlink" style="color: lime;">Pache_Boss22</a></div> -->
							</div>
						</div>
						<div style="display: table-cell; padding: 25px; vertical-align: top; border-left: 1px solid <?php echo $maincolor ?>; ">
							...
							//shop
								...
								?>
								<h1><?php echo uilang("Shop") ?></h1>
								<?php
								if(isset($_POST["newshop"])&& isset($_POST["nomor_wa"])){
									$newshop = mysqli_real_escape_string($connection, $_POST["newshop"]);
									$nomor_wa = mysqli_real_escape_string($connection, $_POST["nomor_wa"]);
									if($newshop != "" && $nomor_wa != ""){
										mysqli_query($connection, "INSERT INTO $tableshop (nama, nomor_wa) VALUES ('$newshop','$nomor_wa')");
										echo "<div class='alert'>" .uilang("New shop has been added"). ".</div>";
									}
								}
								
								if(isset($_GET["deleteshop"])){
									$id = mysqli_real_escape_string($connection, $_GET["deleteshop"]);
									mysqli_query($connection, "DELETE FROM $tableshop WHERE id = $id");
									echo "<div class='alert'>" .uilang("One shop removed"). ".</div>";
								}
								
								//update shop
								if(isset($_GET["updateshop"])){
									?>
									<h3><a href="<?php echo $baseurl ?>admin.php?shop"><i class="fa fa-arrow-left"></i> <?php echo uilang("Back") ?></a></h3>
									<?php
									$id = mysqli_real_escape_string($connection, $_GET["updateshop"]);
									
									if(isset($_POST["newshopupdate"])){
										$newshopupdate = mysqli_real_escape_string($connection, $_POST["newshopupdate"]);
										$nomor_wa_update = mysqli_real_escape_string($connection, $_POST["nomor_wa_update"]);
										if($newshopupdate != "" || $nomor_wa_update != ""){
											mysqli_query($connection, "UPDATE $tableshop SET nama = '$newshopupdate', nomor_wa = '$nomor_wa_update' WHERE id = $id");
											echo "<div class='alert'>" . uilang("Shop updated") . ".</div>";
										}
									}
									...
									?>
									<form method="post">
										<label><?php echo uilang("Enter new name for shop") ?>: <?php echo $row["nama"] ?></label>
										<input type="text" placeholder="<?php echo uilang("Shop") ?>" name="newshopupdate" value="<?php echo $row["nama"] ?>">
										<label><?php echo uilang("Enter new WhatsApp number") ?>: <?php echo $row["nomor_wa"] ?></label>
										<input type="text" placeholder="<?php echo uilang("Shop") ?>" name="nomor_wa_update" value="<?php echo $row["nomor_wa"] ?>">
										<input type="submit" value="<?php echo uilang("Update") ?>" class="submitbutton">
									</form>
									<?php
								}else{
									...
									if(mysqli_num_rows($result) > 0){
										while($row = mysqli_fetch_assoc($result)){
											?>
											<div class="categoryblock"><i class="fa fa-tag"></i> <?php echo $row["nama"] . " | " . $row["nomor_wa"]?> <span style="margin-left: 20px; font-size: 12px; color: black;"><a href="<?php echo $baseurl ?>admin.php?shop&updateshop=<?php echo $row["id"] ?>"><i class="fa fa-edit"></i> <?php echo uilang("Edit") ?></a> | <a href="<?php echo $baseurl ?>admin.php?shop&deleteshop=<?php echo $row["id"] ?>"><i class="fa fa-trash"></i> <?php echo uilang("Delete") ?></a></span></div>
											<?php
										}
									}else{
										echo "<p>" .uilang("No shop has been added"). ".</p>";
									}
									?>
									<br><br>
									<form method="post">
